...
def get_torch_cuda_capabilities() -> Tuple[bool, Optional[str], Optional[List[str]]]:
    ...
    try:
        ...
        logger.warning(
            "PyTorch is not installed. GPU acceleration will not be available."
        )
...
def check_gpu_availability(verbose: bool = True) -> bool:
    ...
    cuda_version_from_smi = (
        get_cuda_version_from_nvidia_smi() if nvidia_smi_available else None
    )

    # Check PyTorch CUDA capabilities
    torch_cuda_available, torch_cuda_version, device_names = (
        get_torch_cuda_capabilities()
    )
    ...
    if verbose:
        if gpu_available:
            ...
            logger.info(
                f"[SUCCESS] GPU found, using CUDA {cuda_version} with {devices_str}"
            )
        else:
            logger.warning(
            ...
            if not nvidia_smi_available:
                logger.info(
                    "   - NVIDIA System Management Interface (nvidia-smi) not found or not working"
                )
