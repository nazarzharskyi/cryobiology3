...
class CellSegKitGUI:
    ...
    def create_segmentation_ui(self, parent):
        ...
        gpu_check = ttk.Checkbutton(
            model_frame, text="Use GPU (if available)", variable=self.use_gpu
        )
        ...
        ttk.Button(dir_frame, text="Browse...", command=self.browse_input_dir).pack(
        ...
        ttk.Checkbutton(
            export_frame, text="Overlay (visualization)", variable=self.export_overlay
        ).pack(anchor=tk.W)
        ttk.Checkbutton(
            export_frame, text="NumPy (data analysis)", variable=self.export_npy
        ).pack(anchor=tk.W)
        ttk.Checkbutton(
            export_frame, text="PNG (indexed masks)", variable=self.export_png
        ).pack(anchor=tk.W)
        ttk.Checkbutton(
        ...
        ttk.Button(action_frame, text="Load Images", command=self.load_images).pack(
            fill=tk.X, pady=(0, 5)
        )
        ttk.Button(
            action_frame, text="Run Segmentation", command=self.run_segmentation
        ).pack(fill=tk.X)

        # Right panel (visualization)
        right_panel = ttk.LabelFrame(
            segmentation_frame, text="Visualization", padding=10
        )
    ...
    def display_current_image(self):
        ...
        self.image_label.config(
            text=f"Image {self.current_image_index + 1} of {len(self.image_paths)}"
        )
        ...
        try:
            ...
            error_label = ttk.Label(
                self.display_frame, text=f"Error displaying image: {str(e)}"
            )
    ...
    def create_conversion_ui(self, parent):
        ...
        left_panel = ttk.LabelFrame(
            conversion_frame, text="Mask Conversion Controls", padding=10
        )
        left_panel.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        ...
        ttk.Button(mask_frame, text="Browse...", command=self.browse_mask_file).pack(
            anchor=tk.W
        )
        ...
        ttk.Label(
            image_frame, text="(Required for overlay and YOLO formats)", font=("", 8)
        ).pack(anchor=tk.W)
        image_entry = ttk.Entry(image_frame, textvariable=self.original_image_path)
        image_entry.pack(fill=tk.X, pady=(0, 5))
        ttk.Button(
            image_frame, text="Browse...", command=self.browse_original_image
        ).pack(anchor=tk.W)
        ...
        format_combo = ttk.Combobox(
            format_frame, textvariable=self.conversion_format, values=format_values
        )
        ...
        ttk.Label(class_frame, text="Class ID (for YOLO format only):").pack(
            anchor=tk.W
        )
        ...
        output_entry.pack(fill=tk.X, pady=(0, 5))
        ttk.Button(
            output_frame, text="Browse...", command=self.browse_conversion_output
        ).pack(anchor=tk.W)

        # Convert button
        convert_button = ttk.Button(
            left_panel, text="Convert Mask", command=self.convert_mask
        )
    ...
    def browse_mask_file(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select Mask File", filetypes=file_types
        )
        if file_path:
    ...
    def browse_original_image(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select Original Image", filetypes=file_types
        )
        if file_path:
    ...
    def browse_conversion_output(self):
        ...
        file_path = filedialog.asksaveasfilename(
            title="Save Output As", filetypes=file_types, defaultextension=default_ext
        )
        if file_path:
    ...
    def _suggest_conversion_output_path(self):
        ...
        output_path = os.path.join(
            directory, f"{filename}_converted_{format_type}{ext}"
        )
    ...
    def convert_mask(self):
        ...
        if output_format in ["overlay", "yolo"]:
            ...
            if not original_image_path:
                messagebox.showerror(
                    "Error", f"Original image is required for {output_format} format"
                )
                return
        ...
        threading.Thread(
            target=self._convert_mask_thread,
            args=(mask_path, output_format, output_path, original_image_path, class_id),
        ).start()

    def _convert_mask_thread(
        self, mask_path, output_format, output_path, original_image_path, class_id
    ):
        try:
            # Run the conversion
            convert_mask_format(
    ...
    def _conversion_complete(self, output_path):
        ...
        messagebox.showinfo(
            "Success", f"Mask conversion complete. Result saved to {output_path}"
        )
